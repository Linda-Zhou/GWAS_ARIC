...
if (length(args)==0) {
  stop("At least two arguments must be supplied - trait and file.", call.=FALSE)
 }
trait<-args[1]
full_filename<-args[2]
...
system(paste0("sed -n '1~20p' ", full_filename, " > allchr.FAST.txt.trim")) 

#to speed up reading gwas input, use sqldf
library(sqldf)
f <- file("allchr.FAST.txt.trim")
...
##qq maf plot##
png(paste(trait,".qqplot.maf.png",sep=""),res=400,units="cm",width = 16, height = 16, type='cairo')
...
##qq imp plot##
png(paste(trait,".qqplot.imp.png",sep=""),res=400,units="cm",width = 16, height = 16, type='cairo')
s<-paste("lambda=",lambda,sep="")
pvals <- data$Pvalue
observed <- sort(pvals)
lobs <- -(log10(observed))
...
##qq eSampleSize plot##
png(paste(trait,".qqplot.effectiveSampleSize.png",sep=""),res=400,units="cm",width = 16, height = 16, type='cairo')
s<-paste("lambda=",lambda,sep="")
pvals <- data$Pvalue
observed <- sort(pvals)
lobs <- -(log10(observed))
...
hits<-subset(data,Pvalue<=0.0001)
write.table(hits, paste(trait,"hits.txt",sep="."),sep="\t",quote=FALSE,row.names=FALSE)
...
png(paste(trait,".manhattanplot.png",sep=""),res=400,width = 32, height = 16,units="cm",type='cairo')
...
#system("rm allchr.FAST.txt.trim")
