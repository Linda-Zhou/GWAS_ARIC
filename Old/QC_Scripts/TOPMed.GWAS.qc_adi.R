...
trait<-args[2]
visit<-args[3]
directory<-paste0("/users/asurapan/04_ARIC.GWAS.with.FAST/",transformation,trait,"_Visit",visit,"_W/")
full_filename<-paste0("/users/asurapan/04_ARIC.GWAS.with.FAST/",transformation,trait,"_Visit",visit,"_W/allchr.FAST.txt")
...
system(paste0("cat ",full_filename, " |awk -F'\t' '$8 > 0.01 && $8 <0.99 && $9 >0.1 && $11<0.1 { print }' > " ,full_filename_trim))
...
system(paste0("rm ",full_filename,".trim.wheader "))
